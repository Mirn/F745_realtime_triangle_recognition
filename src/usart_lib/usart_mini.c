...
void usart_init()
{
	...
	huart6.Init.BaudRate   = USART_BOD;
...
{
	if ((USART6->ISR & USART_ISR_RXNE) != RESET)
	{
		rx_buffer[rx_pos_write % sizeof(rx_buffer)] = USART6->RDR;
		rx_pos_write++;
	}

	if ((USART6->ISR & USART_ISR_ORE) != RESET) {USART6->ICR = USART_ICR_ORECF; rx_errors++;};
...
{
    USART6->TDR = tx_data;
	while ((USART6->ISR & USART_ISR_TC) == RESET);
}
#endif
